...
func GetUserByEmail(email string, db *sql.DB) (*data.User, error) {
	...
	row := db.QueryRow("SELECT id, first_name, last_name, email, password_hash FROM users WHERE email=$1", email)
...
func GetUserByID(id int, db *sql.DB) (*data.User, error) {
	...
	row := db.QueryRow("SELECT id, first_name, last_name, email, password_hash FROM users WHERE id=$1", id)
	err := row.Scan(&user.ID, &user.FirstName, &user.LastName, &user.Email, &user.Password)
